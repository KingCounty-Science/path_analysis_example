...
# mapview(path.res %>% filter(Stat == 'NE') %>% mutate(`COT_min7q` = scale(`COT_min7q`)), cex = 'COT_min7q', col.regions = "#56B4E9", xcol = 'lon', ycol = 'lat', crs = 4269, layer.name = 'NE COT') +
#   # mapview(path.res %>% filter(Stat == 'NE') %>% st_as_sf(coords = c("lon", "lat")) %>% st_jitter(factor = 0.01),
#   mapview(path.res %>% filter(Stat == 'NE') %>% mutate(`MAF_min7q` = scale(`MAF_min7q`), lat = lat-0.05, lon = lon-0.05), 
#           cex = 'MAF_min7q', col.regions = "#E69F00", xcol = 'lon', ycol = 'lat', crs = 4269, layer.name = 'NE MAF')   
# 
# #### try ggplot2
...
tmp <- filter(path.resx, Stat == 'NE'&!Path == 'MAF_COT') %>% mutate(Path = factor(Path, levels = c("MAF_min7q","COT_min7q")))
ggplot(wash_map) + geom_sf(fill="transparent") + 
  theme_bw() +
  geom_sf(data = tmp %>% st_jitter(factor = 0.01), aes(size=Value, color = Path), alpha = 0.5) +
  ggtitle(paste0('Min7Q Flow (Jun 15 - Nov 15) ',yearstart,' to ',yearend)) +
  lims(size = c(0,1)) +
...
#p <- ggplot(path.res %>% filter(Stat == 'NE'), aes(MAF_min7q,COT_min7q, label = staname)) +
  ggplot(path.res %>% filter(Stat == 'NE'), aes(MAF_min7q,COT_min7q, label = staname)) +
  theme_bw() +
  geom_point() +
